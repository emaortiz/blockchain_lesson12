...
// yarn run ts-node --files ./scripts/FinalScript.ts strawberry caramel chocolate
...
async function main () {
    ...
    const deployTransactionReceipt = await contract.waitForDeployment();
    const contractAddress = await deployTransactionReceipt.getAddress();

    console.log(`The Tokenized Vote Contract was deployed at ${contractAddress}`);
    ...
    console.log(`Account 1 has a balance of ${ethers.formatEther(tokenBalanceAccount1)} Vote Tokens`);
    ...
    console.log(`Account 2 has a balance of ${ethers.formatEther(tokenBalanceAccount2)} Vote Tokens`);
    ...
    console.log(`Account 1 has a vote power of ${ethers.formatEther(votePowerAccount1)} units`);
    ...
    console.log(`Tokens delegated from ${account1.address} for ${account1.address} at block ${delegateTxReceipt1?.blockNumber}, cost of ${delegateTxReceipt1?.gasUsed} gas units`);
    ...
    console.log(`Tokens delegated from ${account2.address} for ${account2.address} at block ${delegateTxReceipt2?.blockNumber}, cost of ${delegateTxReceipt2?.gasUsed} gas units`);
    ...
    const transferTx = await contract.connect(account1).transfer(account2.address, MINT_VALUE / 2n);
    ...
    const lastBlockNumber = currentBlock?.number ?? 0;
    console.log(`The current block number is ${lastBlockNumber}`)

    // Check the historical voting power
    votePowerAccount1 = await contract.getPastVotes(account1.address, lastBlockNumber - 1);
    console.log(`Account 1 had a vote power of ${ethers.formatEther(votePowerAccount1)} units at block ${lastBlockNumber - 1}`);
    votePowerAccount1 = await contract.getPastVotes(account1.address, lastBlockNumber - 2);
    console.log(`Account 2 had a vote power of ${ethers.formatEther(votePowerAccount1)} units at block ${lastBlockNumber - 2}`);
    ...
    const ballotContract = await ballotContractFactory.deploy(convertStringArrayToBytes32(proposals), contract.getAddress(), delegateTxReceipt2?.blockNumber ?? 0);
    const deployTxReceipt = await ballotContract.waitForDeployment();
    const addrees = await deployTxReceipt.getAddress();
    console.log(`The Ballot contract was deployed at the address ${addrees}`);

    // 3. CASTING VOTES
    // account1 casts one vote for strawberry, the 0 indexed arg
    console.log("Account 1 votes twice for strawberry")
    const voteTx1 = await ballotContract.connect(account1).vote(0, ethers.parseEther("2"));

    // account1 casts one vote for chocolate, the 2 indexed arg
    console.log("Account 1 votes once for chocolate")
    const voteTx2 = await ballotContract.connect(account1).vote(2, ethers.parseEther("1"));

    // account2 casts five votes for caramel, the 1 indexed arg
    console.log("Account 2 votes five times for caramel")
    const voteTx3 = await ballotContract.connect(account2).vote(1, ethers.parseEther("5"));
    ...
    const winnerFormatted = ethers.decodeBytes32String(winner);
